								...
								"<span class='fc-button fc-button-" + buttonName + " " + tm + "-state-default'>" +
									"<span class='fc-button-inner'>" +
										"<span class='fc-button-content'>" +
											(icon ? "<svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 448 512\"><!--!Font Awesome Free 6.7.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2025 Fonticons, Inc.-->"+ icon + "</svg>":
												text
											) +
										"</span>" +
...
function lazySegBind(container, segs, bindHandlers) {
	container.unbind('mouseover').on("mouseover", function(ev) {
...
function BasicView(element, calendar, viewName) {
	...
	function dayBind(days) {
		...
		if(opt('bindingMode') == 'double')
			days.dblclick(dayClick).on("mousedown", daySelectionMousedown);
		else
			days.on("click", dayClick).on("mousedown", daySelectionMousedown);
	}
...
function AgendaView(element, calendar, viewName) {
	...
	function dayBind(cells) {
		...
		if(opt('bindingMode') == 'double')
			cells.dblclick(daySlotClick).on("mousedown", daySelectionMousedown);
		else
			cells.on("click", daySlotClick).on("mousedown", daySelectionMousedown);
	}

	function slotBind(cells) {
		cells.unbind('click dblclick');
		if(opt('bindingMode') == 'double')
			cells.dblclick(slotClick).on("mousedown", slotSelectionMousedown);
		else
			cells.on("click", slotClick).on("mousedown", slotSelectionMousedown);
	}
...
function AgendaEventRenderer() {
	...
	function renderSlotSegs(segs, modifiedEventId) {
		...
		slotJumpersTop.each(function(i, jumper){
			$(jumper).unbind('click').on("click", function(){
				var targetTop=0;
...
function View(element, calendar, viewName) {
	...
	function opt(name, viewNameOverride) {
		var v = options[name];
		if (typeof v == 'object' && !v.length && !Array.isArray(v)) {
	...
	function eventElementHandlers(event, eventElement) {
		...
		eventElement.find('.fc-event-checkbox').on("click", function(ev) {
